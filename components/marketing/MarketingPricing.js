...
const TIER_ORDER = ["starter", "growth", "custom"];
...
export default function MarketingPricing({ m }) {
  ...
  const tiers = TIER_ORDER.map((id) => {
    ...
    return {
      ...
      cta: t.cta,
      features: t.features,
      ctaVariant: id === "growth" ? "primary" : "secondary",
      featured: id === "growth",
      action: id === "custom" ? "sales" : "checkout",
    };
  });
    ...
    >
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-2xl text-center">
          <h2 id="pricing-heading" className="text-3xl font-semibold tracking-tight text-neutral-950 sm:text-4xl">
            {p.heading}
          </h2>
          <p className="mt-4 text-base leading-relaxed text-neutral-600 sm:text-lg">{p.subheading}</p>
        </div>

        <div className="mt-10 flex justify-center" role="group" aria-label={p.billingAria}>
          <div className="inline-flex items-center gap-1 rounded-full border border-neutral-200/90 bg-white p-1 shadow-sm">
            <span className="rounded-full bg-neutral-950 px-4 py-2 text-sm font-medium text-white">{p.monthly}</span>
            <span className="rounded-full px-4 py-2 text-sm font-medium text-neutral-400" title={p.annualTitle}>
              {p.annual} <span className="text-xs font-normal text-neutral-400">{p.annualSoon}</span>
            </span>
          </div>
        </div>
            ...
            >
              ...
              <div className="flex flex-1 flex-col">
                ...
                <div className="mt-4 flex items-baseline gap-1">
                  ...
                  {tier.period ? <span className="text-sm font-medium text-neutral-500">{tier.period}</span> : null}
                </div>
                ...
                <ul className="mt-8 flex flex-1 flex-col gap-3 text-sm text-neutral-700">
                  {tier.features.map((line) => (
                    <li key={line} className="flex gap-3">
                      <CheckIcon
                        className={
                          tier.featured ? "mt-0.5 shrink-0 text-blue-600" : "mt-0.5 shrink-0 text-neutral-400"
                        }
                      />
        ...
        <p className="mx-auto mt-10 max-w-xl text-center text-xs leading-relaxed text-neutral-500">{p.footerNote}</p>
      </div>
    </section>
  );
}
